...
template<typename A>
double PBMParticlePhase<A>::get_total_processes_rate(const GasPhase& gp, const NucleationTheory& nt) {
...
template<typename A>
double PBMParticlePhase<A>::timestep(double dt, const GasPhase& gp, const NucleationTheory& nt) {
    ...
    if(rho <= R_nucl/R_tot) {
        ...
        consumption += nucleation(j,si)/dt;
...
template<typename A>
void PBMParticlePhase<A>::update_rates(const GasPhase& gp, const NucleationTheory& nt) {
